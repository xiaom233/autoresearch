...
> 合并自 strategy_guide.md + strategy_findings.md + coupling_rules.md
> 基于 ~400 组实验 (exp9 Phase 1-15)
...
**盲预训练最有利于轻退化。退化越重，随机预训练见过类似分布的概率越高→迁移优势越小。**
...
| 严重步骤 | motion 变化 | lens 变化 | 统一方向 |
|----------|:--:|:--:|:--:|
| blur 严重 | Fwd 强化 (-0.58→-2.65) | Rev 消失 (+0.81→+0.01) | **不对称** |
| noise 严重 | Fwd 消失 (-0.58→+0.05) | Rev 强化 (+0.81→+2.47) | **→ Rev** |
| comp 严重 | 未测 | Rev 强化 (+0.81→+1.43) | **→ Rev** |

**机制**：noise/compression 是"覆盖层"（加重→需先剥离外层→Rev），blur 是"结构破坏"（加重→需先修复→因类型而异）。

### 动态课程 split（Phase 11, 13组）

**结论：动态 split 不是普适改进。Fixed 5000/5000/5094 是最稳健默认。**

| 方案 | 最优次数 | 适用场景 |
|------|:--:|------|
| Fixed (均匀) | 6/13 | **最稳健默认** |
| SL (严重度线性) | 5/13 | 特定退化 N12(+3.97), S7(+2.30) |
| TA (类型感知) | 2/13 | T2 突破容量饱和(+2.91), S5(+0.63) |
| PR (峰值比) | 0/13 | **失败**——过度偏好结构化退化 |

**TA 的收益来自正确的类型权重（blur=1.5, noise=0.5），但这些权重是经验猜测，非零成本测量。PR 尝试用频谱峰值比替代经验权重，但因过度分配而失败。**

**规则**：默认用 Fixed split。仅在退化严重不平衡 + 已知瓶颈退化类型时，考虑 TA。
...
| Ft 优势随严重度单调递减 | 5 | 0 | **强** (sev=1-5全测) |
| motion→Fwd 适用范围 sev≤4 | 3 | S3(sev=5翻转) | **强** |
| lens→Rev 全严重度保持 | 2 | 0 | **中** (缺sev=4) |
| 严重步骤主导课程方向 | 5 | S7(不对称) | **中** |
| compression 严重→Rev | 1 | — | **弱** (仅X3) |

...
VAL_COUNT = 5                # max images per val set (0 = use all, small for speed)
...
VAL_INTERVAL = 99999         # steps between full validation (disabled during training)
...
def evaluate(model, val_dataset, device, autocast_ctx):
    """Compute average PSNR over validation set (full images, one at a time)."""
    model.eval()
    psnr_sum = 0.0
    ...
    model.train()
    return psnr_sum / len(val_dataset) if len(val_dataset) else 0.0
...
while True:
    ...
    if step > 0 and step % VAL_INTERVAL == 0:
        val_psnr = evaluate(model, val_dataset, device, autocast_ctx)

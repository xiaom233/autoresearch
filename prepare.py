...
def load_degradation_params(params_path):
    """Load degradation pipeline from params.json generated by image-degradation-simulator.

    The params.json contains a 'pipeline' array where each entry has:
        {"step": 1, "function": "blur_gaussian", "severity": 3, ...}

    Args:
        params_path (str): Path to params.json.
...
def make_dataloader_restoration(params_path, shards_url, batch_size,
    ...
    Args:
        params_path (str): Path to params.json from image-degradation-simulator skill.
    ...
    def decode_and_degrade(sample):
        ...
        for ext in ('png', 'jpg', 'jpeg', 'ppm', 'webp'):
            if ext in sample:
                ...
                clean = img.copy()
                for func_name, severity in pipeline:

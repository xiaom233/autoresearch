...
盲识别必须通过 `Skill(skill="image-degradation-simulator", ...)` 调用。Skill 强制执行：视觉检查 → reflection.json → 假设驱动 → 视觉对比纹理 → **保存前质量校验** → save_results.py。

### ⚠️ CI Pass 不等于盲识别正确 (exp13 验证)

同图模式下 CI (compare_degradation.py) 只测量**视觉相似性**，不测量**函数正确性**：
...
exp13 数据：CI=8/10 的退化中有函数 0/2 完全错误的情况。

**结论**：CI 是模拟退化的收敛信号，**不能**用作"盲识别成功"的唯一判据。Skill 必须在保存前执行管线结构校验 + 已知误识别模式检查。训练后的 Spec vs M_blind PSNR 差是更可靠的反思触发条件。
